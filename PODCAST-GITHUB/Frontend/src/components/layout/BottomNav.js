...
const BottomNav = () => {
	...
	return (
		<div className="fixed bottom-0 z-50 left-0 w-full bg-gray-800 shadow-md border-t border-gray-200">
			<div className="grid grid-cols-3 text-center h-20 items-center">
				{footerItems.map((item, index) => (
					...
					>
						{item.icon}
						<span className="text-xs sm:text-sm font-bold text-white mt-1">{item.subTitle}</span>
					</div>
				))}
			</div>
		</div>
